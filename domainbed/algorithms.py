...
class PrecomputedHOGMLP(Algorithm):
    """
    PrecomputedHOGMLP trains a small multi-layer perceptron on precomputed HOG features.
    It expects input features as flat vectors.
    """
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(PrecomputedHOGMLP, self).__init__(input_shape, num_classes, num_domains, hparams)
        # Calculate input dimension (should be already a flat tuple like (hog_dim,))
        ...
        hidden_dim = hparams.get("mlp_hidden_dim", 512)
        n_layers = hparams.get("mlp_num_layers", 2)  # if 2, one hidden layer + output layer
    ...
    def update(self, minibatches, unlabeled=None):
        # Each minibatch is a tuple (features, labels). Combine them.
        all_x = torch.cat([x for (x, y) in minibatches])
        all_y = torch.cat([y for (x, y) in minibatches])
