...
class HOGPACS(MultipleDomainDataset):
    """PACS dataset using precomputed HOG features instead of images."""
    CHECKPOINT_FREQ = 300  # same as PACS
    ENVIRONMENTS = ["A", "C", "P", "S"]  # domain identifiers matching saved files

    def __init__(self, root, test_envs, hparams):
        super().__init__()
        if root is None:
            raise ValueError("Data directory not specified for HOGPACS!")
        # Directory containing precomputed feature files (assume under root)
        self.dir = os.path.join(root, "PACS_HOG")
        if not os.path.isdir(self.dir):
            raise FileNotFoundError(f"HOG features directory not found: {self.dir}")
        self.datasets = []  # list of TensorDataset for each domain

        for i, env_name in enumerate(self.ENVIRONMENTS):
            feature_file = os.path.join(self.dir, f"{env_name}.pt")
            # Load precomputed features and labels for this environment
            data_tuple = torch.load(feature_file)
            # Expect data_tuple to be (features_tensor, labels_tensor)
            features_tensor, labels_tensor = data_tuple if isinstance(data_tuple, tuple) else (
                data_tuple["features"], data_tuple["labels"])
            # Create a dataset for this environment
            env_dataset = TensorDataset(features_tensor, labels_tensor)
            self.datasets.append(env_dataset)
        # Set input shape and number of classes for DomainBed
        self.input_shape = (features_tensor.shape[1],)  # e.g. (hog_dim,)
        self.num_classes = int(torch.max(labels_tensor).item() + 1)  # or simply 7 for PACS
