...
def _hparams(algorithm, dataset, random_seed):
    ...
    # In domainbed/hparams_registry.py (inside the _hparams function definition):

    elif algorithm == "PrecomputedHOGMLP":
        # Do not re-define lr or weight_decay if they are already set in the unconditional block.
        # _hparam("batch_size", 32, lambda r: int(2**r.uniform(3, 5.5)))
        _hparam("mlp_hidden_dim", 512, lambda r: int(r.choice([256, 512, 1024])))
        _hparam("mlp_num_layers", 2, lambda r: int(r.choice([1, 2, 3])))
        _hparam("mlp_dropout", 0.0, lambda r: float(r.choice([0.0, 0.1, 0.5])))
